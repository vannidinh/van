...
</nav>       <br><br> <div class="container"> <form action="addDrink.php" method="post" class="form-signin">
                <h1 class="form-signin-heading">Enter Drink</h1>
...
?>                
                ...
                <div>
              <label for="name">Topseller:</label>
              <input type="topseller" name="topseller" class="form-control"  />
              </div>
              <div><br>
...
include "dbConnection.php";
  ...
  $sql = "SELECT ID,flavor FROM drinks";

  $result = $conn->query($sql);

    if ($result->num_rows > 0) {  
      
$select= '<select name="flavor">';
while($rs = $result->fetch_assoc()){
     $select.='<option value="'.$rs['ID'].'">'.$rs['flavor'].'</option>';
  }
}
$select.='</select>';
echo $select;
?>
              ...
              <div>
                  ...
                  <input type="text" name="nutritional value" class="form-control"  />
              </div>
